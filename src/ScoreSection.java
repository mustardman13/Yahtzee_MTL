...
public class ScoreSection extends GCompound{

	/* Basic constructor using the default color scheme*/
	public ScoreSection(double x, double y) {
		...
		// Initialize the scores. Use -1 as a flag that a valid score has
	...
	public boolean updatePlayerNames (String[] playerName) {
	 ...
	 * the player number is an invalid number */
	public boolean updatePlayerColors (ColorScheme_Dice[] playerColors) {
	...
	/* subroutine to determine what score options are applicable. Could not get the buttons in this
	 * module to work. Will now try to pass the score information to the main module and run the buttons
	 * from there. category corresponds with the row number*/
	public int getScoreOption (int playerNumber, int categoryNumber, Die[] dice) {
		...
		/* Don't need to evaluate all options, just the one corresponding with the one passed to this function. With each check first whether
		 * that category has already been utilized. Otherwise return "-1" as a flag that this is not a valid option.*/ 
		switch (categoryNumber) {
			case ROW_NUM_ONES:
				// Ones category	
				if (scores[ROW_NUM_ONES][playerNumber]==-1) {
					return countOnes;
				} else {
					return -1;
				}
			case ROW_NUM_TWOS:
				// Twos category
				if (scores[ROW_NUM_TWOS][playerNumber]==-1) {
					return 2*countTwos;
				} else {
					return -1;
				}
			case ROW_NUM_THREES:
				// Threes category
				if (scores[ROW_NUM_THREES][playerNumber]==-1) {
					return 3*countThrees;
				} else {
					return -1;
				}
			case ROW_NUM_FOURS:
				// Fours category
				if (scores[ROW_NUM_FOURS][playerNumber]==-1) {
					return 4*countFours;
				} else {
					return -1;
				}
			case ROW_NUM_FIVES:
				// Fives category
				if (scores[ROW_NUM_FIVES][playerNumber]==-1) {
					return 5*countFives;
				} else {
					return -1;
				}
			case ROW_NUM_SIXES:
				// Sixes category
				if (scores[ROW_NUM_SIXES][playerNumber]==-1) {
					return 6*countSixes;
				} else {
					return -1;
				}
			case ROW_NUM_THREE_OF_A_KIND:
				if (scores[ROW_NUM_THREE_OF_A_KIND][playerNumber]==-1) {
			...
			case ROW_NUM_FOUR_OF_A_KIND:
				if (scores[ROW_NUM_FOUR_OF_A_KIND][playerNumber]==-1) {
			...
			case ROW_NUM_FULL_HOUSE:
				if (scores[ROW_NUM_FULL_HOUSE][playerNumber]==-1) {
			...
			case ROW_NUM_SM_STRAIGHT:
				if (scores[ROW_NUM_SM_STRAIGHT][playerNumber]==-1) {
			...
			case ROW_NUM_LG_STRAIGHT:
				if (scores[ROW_NUM_LG_STRAIGHT][playerNumber]==-1) {
			...
			case ROW_NUM_YAHTZEE:
				...
				if (scores[ROW_NUM_YAHTZEE][playerNumber]!=0) {
					if ((countOnes >= 5 || countTwos >= 5 || countThrees >= 5 || countFours >= 5 || countFives >= 5 || countSixes >= 5)) { 
						if (scores[ROW_NUM_YAHTZEE][playerNumber] == -1) {
							return 50;
						} else {
							return scores[ROW_NUM_YAHTZEE][playerNumber] + 50;
						}
					} else {
						return -1;
					}
			...
			case ROW_NUM_CHANCE:
				if (scores[ROW_NUM_CHANCE][playerNumber]==-1) {
					/* Chance is available until it is used. There is not criteria other than only
					 * being able to use it once. */
					return sum;
				}
			default:
				// Return -1. This gives the calling function a flag to indicate that the value should not be displayed.
				return -1;
		}
	}
	...
	public void displayScoreOption (int playerNumber, Die[] dice) {
...
/* ---------------------- Task to complete ----------------------- */
	 ...
	 * For now the only thing we are looking for is to see if it
	 * is a score option being selected. */
	public void mousePressed(MouseEvent e) {
		
		GPoint clickPoint = new GPoint(e.getX(),e.getY());
		
		/* Start with the first non-header row.*/
		for (int i = N_HEADER_ROWS; i < N_TOTAL_ROWS; i++) {
			if ( scoreGrid[i][SCORE_OPTION_COL].contains(clickPoint) ) {
				
				/* Look at the value of the score optoin box. If it is visible then assume
				 * it is a valid selection. */
				
				if (scoreGridLabel[i][SCORE_OPTION_COL].isVisible() == true) {
					/* Assign the score and clear the options */
					scores[i][SCORE_OPTION_COL] = scoresOptions[i][SCORE_OPTION_COL];
					hideOptions();
...
//	private GLabel[][] scoreGridLabel = new GLabel[GRID_ROWS][GRID_COLS];	// Use a 2-D array of points as
	...
	private GRoundRect[][] scoreGrid = new GRoundRect[19][GRID_COLS];
	private GLabel[][] scoreGridLabel = new GLabel[19][GRID_COLS];
	...
	private int[][] scores = new int[GRID_ROWS][N_PLAYERS];
	private int[][] scoresOptions = new int[GRID_ROWS][N_PLAYERS];
